'use client';
// FILE: src/components/leadershipReview/IssueInfoPanel.tsx

import Link from 'next/link';
import type { LeadershipReviewIssue } from '@/types/leadershipReview';

// NOTE: SubscribeForm has been moved out of this component.
// It is now rendered in IssueDetailClient, positioned after ShareAndSupportCard,
// so it appears below the share buttons in the sidebar as requested.
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: '16px',
        boxShadow: '0 8px 32px rgba(40,53,131,0.13)',
      }}
    >
      {/* Header */}
      ...
      <div className="bg-white p-4 sm:p-5 flex flex-col gap-3 sm:gap-4">
        ...
        {issue.featuredLeader && (
          <div className="pt-3" style={{ borderTop: '1.5px solid #3fa53530' }}>
            ...
            <p className="text-sm font-black" style={{ color: '#283583' }}>
              {issue.featuredLeader}
            </p>
            {issue.leaderTitle && (
              <p className="text-xs text-gray-500 mt-0.5">{issue.leaderTitle}</p>
            )}
            {issue.county && (
